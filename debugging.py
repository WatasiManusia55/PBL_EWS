...
CORRUPTED_PACKET_LOG = "corrupted_packets.json"

def save_corrupted_packet(raw_packet, error_msg, rssi, snr):
    ...
    try:
        ...
        log_entry = {
            ...
            "packet_size": len(str(raw_packet)),
            "rssi": int(rssi),
            "snr": float(snr),
            "error": str(error_msg)
        }

        with open(CORRUPTED_PACKET_LOG, "a") as f:
            f.write(json.dumps(log_entry) + "\n")

        print("📝 Corrupted packet saved")

    except Exception as e:
        print("❌ Failed saving corrupted packet:", e)
...
while True:
    try:
        ...
        if packet is not None:
            try:
                if rfm9x.crc_error:
                    print("⚠️ CRC Error detected in received packet")
                    continue
            except:
                pass
            packet_size = len(packet)
            try:
                raw_packet_string = packet.decode(
                    "utf-8", errors="replace"
                ).strip()
                print("📦 RAW RX:", raw_packet_string)
                if len(raw_packet_string) < 5:
                    print("⚠️ Packet terlalu pendek")
                    continue

                if "{" not in raw_packet_string:
                    print("⚠️ Bukan format JSON")
                    continue

                # parse dengan smart parser yang lebih toleran
                data = parse_packet(raw_packet_string)
                if not data:
                    save_corrupted_packet(
                        raw_packet=raw_packet_string,
